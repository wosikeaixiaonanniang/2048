...
void GameUI::drawBoxes(int offsetY, string name, bool issignin)
{
	...
	{
		if (you != NULL)
			sprintf(yourtext, "您的最佳分数为:%d    使用步数:%d    排名为:%d", you->score, you->step, you->rank);//文字内容
	...
	for (int i = 0; i < BOX_COUNT; i++)
	{
		...
		if (!end)
		{
			sprintf(text, "%s的分数为:%d    使用步数:%d    排名为:%d", head->name.c_str(), head->score, head->step, head->rank);
