...
void drawBoxes(int offsetY,string name) 
{
	...
	if (name != "guest")
	{
		sprintf(yourtext, "您的最佳分数为:%d	使用步数	排名为:%d", you->score, you->step, you->rank);//文字内容
	...
	for (int i = 0; i < BOX_COUNT; i++) 
	{
		...
		if (!end)
		{
			sprintf(text, "%s的分数为:%d    使用步数:%d		排名为:%d", head->name, head->score, head->step, head->rank);
...
void rankmenu() 
{
	...
	while (1) 
	{
		BeginBatchDraw();
		cleardevice();
		drawBoxes(scrollOffset,playername);
